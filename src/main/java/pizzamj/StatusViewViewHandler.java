...
public class StatusViewViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenRefunded_then_UPDATE_5(@Payload Refunded refunded) {
        try {
            if (refunded.isMe()) {
                // view 객체 조회
                List<StatusView> statusViewList = statusViewRepository.findByOrderId(refunded.getOrderId());
                for(StatusView statusView : statusViewList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    statusView.setRefundReason(refunded.getReason());
                    // view 레파지 토리에 save
                    statusViewRepository.save(statusView);
                }
            }
